...
// In Cloud Functions, use functions.config() or process.env if set via CLI
const openai = new OpenAI({
    apiKey: process.env.VITE_API_KEY || functions.config().openai?.key
});

// Simple in-memory cache for API responses
// Note: In Cloud Functions, memory is ephemeral but can persist across warm invocations
const cache = new Map();
...
app.use((req, res, next) => {
    console.log(`[Request] Path: ${req.path}, OriginalUrl: ${req.originalUrl}, Method: ${req.method}`);
    next();
});
...
router.get('/health', (req, res) => {
    res.json({ status: 'OK', message: '3I/ATLAS Backend API Running on Firebase' });
});

// JPL Horizons API endpoint - Fetch live ephemeris data
router.get('/jpl-horizons/:objectId', async (req, res) => {
    try {
...
router.get('/solar-system-positions', async (req, res) => {
    try {
...
router.get('/mpc-observations/:designation', async (req, res) => {
    try {
...
router.post('/mission-briefing', async (req, res) => {
    try {
...
// Mount the router on both /api and root to handle different hosting configurations
app.use('/api', router);
app.use('/', router);
